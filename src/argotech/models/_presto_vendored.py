"""Vendored verbatim from nasaharvest/presto @ `single_file_presto.py` (MIT, (c) 2024 Presto
Authors — see https://github.com/nasaharvest/presto/blob/main/LICENSE).

Vendored rather than installed: the published package pins `torch==2.0`, `numpy==1.23.5` and
`earthengine-api`, and its `__init__` imports `ee` transitively, so even `pip install --no-deps`
cannot import it. The authors ship this single file precisely for that case — it needs only torch
and einops. Do not edit; replace wholesale when upstream changes.
"""
